...
def create_triggers(cur):
    ...
    cur.execute("""CREATE TRIGGER IF NOT EXISTS update_highest_streak
    ...
    cur.execute("""CREATE TRIGGER IF NOT EXISTS update_current_streak
                    AFTER INSERT ON trainee_records
                    BEGIN
                        UPDATE trainees
    ...
    cur.execute("""CREATE TRIGGER IF NOT EXISTS handle_broken_streak
                    AFTER INSERT ON trainee_records
                    BEGIN
                        UPDATE trainees
    ...
    cur.execute("""CREATE TRIGGER IF NOT EXISTS update_total_days
                    AFTER INSERT ON trainee_records
                    BEGIN
                        UPDATE trainees
    ...
    cur.execute("""CREATE TRIGGER IF NOT EXISTS update_total_problems
                    AFTER INSERT ON trainee_records
                    BEGIN
                        UPDATE trainees
...
def create_tables_if_not_exist():
    ...
    if not trainees_exists:
        cur.execute("""CREATE TABLE trainees (
    ...
    if not trainee_records_exists:
        cur.execute("""CREATE TABLE trainee_records (
                    discord_id TEXT ,
                    discord_name TEXT,
                    post_date TEXT,
                    message TEXT,
...
def add_record(record_info):
    ...
    try:
        ...
        cur.execute(
            "SELECT COUNT(*) FROM trainees WHERE discord_id = ?", (discord_id,))
        if cur.fetchone()[0] == 0:
            # If discord_id is not associated with a trainee, add it with default values
            cur.execute("""INSERT INTO trainees (discord_id, discord_name, highest_streak, current_streak, total_days, total_problems)
                           VALUES (?, ?, 0, 0, 0, 0)""", (discord_id, discord_name))
            print(f"added {discord_name} to the database")

        # Get the current date
        if streak != 0:
            cur.execute(
                "SELECT MAX(post_date) FROM trainee_records WHERE discord_id = ?", (discord_id,))
