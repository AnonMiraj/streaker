...
async def archive(ctx):
    ...
    async with ctx.typing():
        ...
        async for message in channel.history(limit=None):
            ...
            print(data)
            if data.get("streak")!=None :
                messages_data.append([message.author.id,message.author.name, message.content,str(message.created_at.isoformat()),data["streak"],data["days"],data["problems"],(data["today"])])
        # TODO 
        filename = f"{channel.name}-archive.csv"
        with open(filename, "w", newline="", encoding="utf-8") as file:
            writer = csv.writer(file)
            writer.writerow(["Author_id","Author_name", "Message","Message date","streak","days","problems","practiced_day"]) 

            """
            [Author_id                        Author_name                               message                                   message date                                      streak                    days                problems             practiced_day
